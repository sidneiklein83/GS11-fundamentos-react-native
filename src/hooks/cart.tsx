...
const CartProvider: React.FC = ({ children }) => {
  ...
  const delToCart = useCallback(
    id => {
      setProducts(productsPrevState => {
        return productsPrevState.filter(f => f.id !== id);
      });
    },
    [setProducts],
  );
  ...
  const value = React.useMemo(
    () => ({ addToCart, delToCart, increment, decrement, products }),
    [products, addToCart, delToCart, increment, decrement],
  );
